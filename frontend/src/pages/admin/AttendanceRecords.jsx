...
const AttendanceRecords = () => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            // Pre-load all signatures with robust error handling
            const signatures = await Promise.all(
                recordsToExport.map(async (reg) => {
                    ...
                    const sigUrl = reg.signature.startsWith('data:')
                        ? reg.signature
                        : `${backendUrl}/uploads/${reg.signature}`;
            ...
            const body = recordsToExport.map((reg, index) => {
                if (isTeamEvent) {
            ...
            let finalY = (doc.lastAutoTable ? doc.lastAutoTable.finalY : 70) + 35;
