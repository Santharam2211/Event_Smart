    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(eventData).forEach(key => {
                if (key !== 'bannerImage' && key !== 'registrationForm' && key !== 'feedbackForm') {
                                        ...
                                        <div className="flex-1 space-y-4">
                                            ...
                                            {['dropdown', 'radio', 'checkbox'].includes(field.type) && (
                                                <div className="flex gap-2 items-center bg-white p-2 rounded-lg border border-primary-100">
                                                    ...
                                                    <input 
                                                        type="text" 
                                                        placeholder="Enter options separated by commas (e.g. Excellent, Good, Average)" 
                                                        className="flex-1 text-sm bg-transparent border-none focus:ring-0 p-1"
                                                        value={field.options ? field.options.join(', ') : ''}
                                                        onChange={(e) => updateFeedbackField(index, 'options', e.target.value.split(',').map(s => s.trim()))}
                                                    />
                                                </div>
                                            )}
                                        </div>
