...
exports.markAttendance = async (req, res, next) => {
    try {
        ...
        const registration = await Registration.findOne({ 
            registrationId, 
            event: eventId 
        }).populate('participant', 'username email signature').populate('team');

        if (!registration) {
            res.status(404);
            throw new Error('Invalid registration or event mismatch');
        }
        ...
        if (registration.team) {
            // Check if scanning user is the Team Leader
            if (registration.team.leader.toString() !== registration.participant._id.toString()) {
                res.status(403);
                throw new Error('Only the Team Leader\'s QR code or ID can mark attendance for the team');
            }
...
exports.getAttendanceRecords = async (req, res, next) => {
    try {
        ...
        const registrations = await Registration.find({ event: req.params.eventId })
            .populate('participant', 'username email registrationNumber phone')
            .populate('team', 'name')
